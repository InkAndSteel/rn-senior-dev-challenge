...
class DatabaseService {
  ...
  async createUser(email: string, password: string): Promise<SQLite.SQLiteRunResult> {
    if (!this.db) throw new Error("Database not initialized");

    const result = await this.db.runAsync("INSERT INTO users (email, password) VALUES (?, ?)", [email, password]);
    return result;
  }
